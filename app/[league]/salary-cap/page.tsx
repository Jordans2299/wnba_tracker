...
import { formatCurrency, teamUrl, isLeague, leagueLabel, leagueCap, LEAGUES, SITE_URL } from "@/lib/utils";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${label} Salary Cap (${year}) - Team Payroll Rankings`;
  const description = `${label} salary cap for ${year} is ${formatCurrency(leagueCap(params.league))}. See every team's total payroll, cap room, and guaranteed salary ranked by spending.`;
  return {
    title,
    description,
    alternates: { canonical: `${SITE_URL}/${params.league}/salary-cap` },
    openGraph: { title, description, url: `${SITE_URL}/${params.league}/salary-cap` },
  };
}

export default async function SalaryCap({ params }: Props) {
  ...
  const teamRows = Object.values(teamSummaries)
    .map((t) => {
      ...
      const cap = t.salaryCap ?? CAP;
      const room = t.capRoom ?? Math.max(0, cap - total);
      return { ...t, cap, total, room, roster };
    })
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <header className="mb-6">
          ...
          <p className="mt-3 text-sm text-court-300 max-w-2xl leading-relaxed">
            The {meta.season} {label} salary cap is <strong className="text-white">{formatCurrency(CAP)}</strong> per team.
        ...
        <div className="overflow-hidden rounded-xl border border-white/5 bg-white/[0.02]">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {teamRows.map((t, i) => {
                  const capPct = Math.round((t.total / t.cap) * 100);
                  return (
                    <tr
                      key={t.urlSlug}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-medium">
                        <Link href={teamUrl(t.name, league)} className="text-white hover:text-accent transition-colors">
                          {t.name}
                        </Link>
                      </td>
