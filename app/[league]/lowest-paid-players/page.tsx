...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Lowest Paid ${label} Players (${year})`;
  const description = `${label} players with the lowest salaries in ${year}. See minimum wage contracts and entry-level player pay across all teams.`;
  return {
    title,
    description,
    alternates: { canonical: `${SITE_URL}/${params.league}/lowest-paid-players` },
    openGraph: { title, description, url: `${SITE_URL}/${params.league}/lowest-paid-players` },
  };
}

export default async function LowestPaidPlayers({ params }: Props) {
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <header className="mb-6">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-white">
            Lowest Paid {label} Players
          </h1>
          <p className="mt-3 text-sm text-court-300 max-w-2xl leading-relaxed">
            The minimum {label} salary in {meta.season} is{" "}
